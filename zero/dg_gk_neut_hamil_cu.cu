...
gkyl_dg_gk_neut_hamil_set_cu_dev_ptrs(struct gkyl_dg_gk_neut_hamil *up, 
  ...
  const struct gkyl_array* gij, struct gkyl_array* hamil)
{
  ...
  gkyl_dg_gk_neut_hamil_calc_cu_kernel<<<nblocks, nthreads>>>(up->on_dev,
...
{    
  ...
  struct gkyl_dg_gk_neut_hamil *up_cu = (struct gkyl_dg_gk_neut_hamil*) gkyl_cu_malloc(sizeof(*up_cu));
  gkyl_cu_memcpy(up_cu, up, sizeof(gkyl_dg_gk_neut_hamil), GKYL_CU_MEMCPY_H2D);

  gkyl_dg_gk_neut_hamil_set_cu_dev_ptrs<<<1,1>>>(up_cu, b_type, cdim, vdim, poly_order);
