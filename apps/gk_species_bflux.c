...
{ 
  ...
  for (int i=0; i<app->cdim; ++i) {
    ...
    bflux->integ_moms[2*i] = gkyl_dg_updater_moment_gyrokinetic_new(&bflux->boundary_grid[2*i],
      &app->confBasis, &app->basis, &bflux->conf_r[2*i], s->info.mass, s->vel_map, app->gk_geom, "M0", true, app->use_gpu);
    bflux->integ_moms[2*i+1] = gkyl_dg_updater_moment_gyrokinetic_new(&bflux->boundary_grid[2*i],
      &app->confBasis, &app->basis, &bflux->conf_r[2*i+1], s->info.mass, s->vel_map, app->gk_geom, "M0", true, app->use_gpu);
